...
public abstract class Pelicula implements IPelicula {
    ...
    public Pelicula(String nombre,Integer anho,Integer duracion,IDirector director,HashMap<String,Float> recaudacion) throws ExcepcionPelicula{

        //comprobamos el nombre de la pelicula

        if(nombre==null){
            throw new ExcepcionPelicula("No se puede crear la pelicula: el nombre de la pelicula no puede ser null",this);
        }else this.nombre=nombre;

        //comprobamos el año
        //buscamos si se produce un fallo en el setter si se produce lanzamos excepccion de que el fallo se produjo en el constructor

        try {
            setAnho(anho);
        }catch (ExcepcionPelicula error){
            //Si manejamos el error lanzamos una nueva excepccion para saber que se produjo en el constructor
            throw new ExcepcionPelicula("No se puede crear la pelicula: "+anho+" no esta en el rango de 1895 y 2022",this);
        }
        ...
        try {
            ...
            //Si manejamos el error lanzamos una nueva excepccion para saber que se produjo en el constructor
            throw new ExcepcionPelicula("No se puede crear la pelicula:"+this.nombre+":"+anho+"no esta en el rango de 1895 y 2022",this);
        }
        ...
        try {
            ...
            throw new ExcepcionPelicula("No se puede crear la pelicula el director de la pelicula no puede ser null",
                    this);
        }
    ...
    public void setAnho(Integer anho) throws ExcepcionPelicula{

        if (anho<1985 || anho>2022){
            throw new ExcepcionPelicula("No se puede modificar el año de la pelicula "+this.nombre+":"+anho+" no está en el rango de 1895 y 2022",
                    this);
        }
        this.anho=anho;
    }

    public void setDuracion(Integer duracion) throws ExcepcionPelicula {
        //como no es primitivo tenemos que comprobar
        if(duracion==null || duracion <= 5) {
               throw new ExcepcionPelicula("No se puede modificar la duración de la película "+this.nombre+":"+ duracion+" no es mayor de 5",
                       this);
        }
